...
var STORAGE={TIMERS:'timeflow_timers',SETTINGS:'timeflow_settings',NOTIFIED:'timeflow_notified',DND:'timeflow_dnd',DRAG_TIP:'chronoflow_drag_tip_version',IOS_TIP:'chronoflow_ios_tip_shown'};
var MODAL={TIMER:'timerModal',FILTER:'fModal',SETTINGS:'sModal',CONFIRM:'confirmModal'};
var CATEGORIES=[{id:'work',name:'Работа'},{id:'health',name:'Здоровье'},{id:'personal',name:'Личное'},{id:'study',name:'Учёба'},{id:'travel',name:'Путешествия'}];
var CAT_NAMES={};CATEGORIES.forEach(function(c){CAT_NAMES[c.id]=c.name});
...
var TEXTS={
emptyTitle:'Начните считать время',
emptyBody:'Не курю 45 дней. До отпуска 128 дней. На новой работе 2 года. Один счётчик — и вы видите прогресс.',
emptyBtn:'Создать счётчик',
noResultsTitle:'Ничего не найдено',
noResultsBody:'Попробуйте изменить фильтр или поиск.',
noResultsBtn:'Сбросить фильтры',
newTimer:'Новый счётчик',
editTimer:'Редактировать',
added:'Добавлено',
updated:'Обновлено',
deleted:'Удалено',
restored:'Восстановлено',
duplicated:'Счётчик дублирован',
cleared:'Очищено',
muted:'Уведомления отключены на 24 ч',
unmuted:'Уведомления включены',
noConnection:'Нет соединения',
connectionBack:'Соединение восстановлено',
refreshed:'Обновлено',
...
noData:'Нет данных',
noTimers:'Нет счётчиков',
importError:'Ошибка импорта: ',
updateAvailable:'Доступно обновление. Обновите страницу.',
...
_lastHourSeg:-1,_lastAccent:'',_lastMoonDay:-1,
_cachedNearest:null,_cachedAccent:'',_cachedNearestCalc:0,
_lastTimersChange:0
};
...
function localISO(d){var off=d.getTimezoneOffset();return new Date(d.getTime()-off*60000).toISOString().slice(0,16)}
...
var knownNewMoon=new Date(2000,0,6,18,14).getTime();
var synodic=29.530588853*DAY_MS;
var days=(date.getTime()-knownNewMoon)/synodic;
var phase=days-Math.floor(days);
...
if(t.type!=='countdown'||t.date<=now)continue;
if(!nearest||t.date<nearest.date)nearest=t;
}
return nearest;
}

function computeAccentFromNearest(nearest){
if(nearest&&(nearest.date-Date.now())<30*DAY_MS)return nearest.color;
return COLORS[relDay(new Date())];
}

function refreshAccentCache(){
state._cachedNearest=getNearestCountdown();
state._cachedAccent=computeAccentFromNearest(state._cachedNearest);
state._cachedNearestCalc=Date.now();
}
...
if(e.name==='QuotaExceededError'||e.code===22){toast.error('Хранилище переполнено. Удалите лишние счётчики.')}
}
}
};
...
return t&&typeof t==='object'&&typeof t.id==='string'&&typeof t.title==='string'&&typeof t.date==='number';
}):[];
}

function saveTimers(){
storage.set(STORAGE.TIMERS,state.timers);
state._lastTimersChange=Date.now();
state._cachedNearestCalc=0;
}
...
function notify(timer,body,type){toast.show(timer.title+' — '+body,type)}
...
state.notified[key]=true;changed=true;
...
state.notified[lastPraiseKey]=milestone;changed=true;
...
function buildBadge(className,label){return '<span class="badge '+className+'">'+esc(label)+'</span>'}
...
var h=$('emptyTitle'),p=$('emptyBody'),b=el.emptyBtn;
if(!h||!p||!b)return;
if(kind==='noResults'){
...
el.timerList.innerHTML='';
if(state.groupByCategory&&state.sortBy==='order'&&filtered.length>1){renderGrouped(filtered)}
...
hdr.innerHTML='<span class="cat-name">'+esc(catName)+'</span><span class="cat-count">'+items.length+'</span><span class="cat-toggle'+(isOpen?' open':'')+'">▶</span>';
...
if(id===MODAL.TIMER){var hp=$('hslPopup');if(hp)hp.classList.remove('show')}
}
var nextId=state._modalStack[state._modalStack.length-1];
if(nextId){trapFocus($(nextId))}
else {
releaseFocus();
...
if(state._focusTrap){document.removeEventListener('keydown',state._focusTrap);state._focusTrap=null}
var main=document.querySelector('main');
...
state.editId=null;
el.mTitle.textContent=TEXTS.newTimer;
el.timerTitle.value='';
el.timerDate.value='';
...
if(!t)return;
state.editId=id;
el.mTitle.textContent=TEXTS.editTimer;
...
state._undoTimer=setTimeout(function(){t.classList.remove('show');state._undoData=null},5000);
}
...
download(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),'chronoflow-'+new Date().toISOString().slice(0,10)+'.json');
...
c.fillText(formatDate(Date.now()).split(' ')[0],400,90);
...
svg.date=$('svgDate');
svg.st=$('svgSt');
svg.hourSegments=$('hourSegments');
svg.eventArrow=$('eventArrow');
svg.activeDot=$('activeEventDot');
svg.moon=$('moonPhase');

if(svg.ring){svg.ring.style.strokeDasharray=SEC_RING_CIRC;svg.ring.style.strokeDashoffset=SEC_RING_CIRC}

if(svg.hourSegments){
svg.hourSegments.innerHTML='';
var R=76;
for(var i=0;i<12;i++){
var startA=-90+i*30+2;
var endA=-90+(i+1)*30-2;
var path=document.createElementNS(SVG_NS,'path');
path.setAttribute('d',describeArc(100,100,R,startA,endA));
path.setAttribute('stroke-width','4');
path.setAttribute('fill','none');
path.classList.add('future');
svg.hourSegments.appendChild(path);
}
}
...
state._lastHourSeg=-1;
state._lastAccent='';
state._lastMoonDay=-1;
state._cachedNearestCalc=0;
}

function updateEventArrow(){
if(!svg.eventArrow)return;
var nearest=state._cachedNearest;
if(!nearest){svg.eventArrow.setAttribute('opacity','0');return}
var daysLeft=Math.max(0,(nearest.date-Date.now())/DAY_MS);
var angle=Math.min(daysLeft/365,1)*180;
svg.eventArrow.setAttribute('transform','rotate('+angle.toFixed(2)+' 100 100)');
svg.eventArrow.setAttribute('opacity','0.85');
}

function updateActiveEventDot(){
if(!svg.activeDot)return;
var now=Date.now();
var isActive=false;
for(var i=0;i<state.timers.length;i++){
if(Math.abs(state.timers[i].date-now)<30*60*1000){isActive=true;break}
}
if(isActive){
svg.activeDot.setAttribute('r','4');
svg.activeDot.setAttribute('opacity','1');
svg.activeDot.classList.add('active');
} else {
svg.activeDot.setAttribute('r','0');
svg.activeDot.setAttribute('opacity','0');
svg.activeDot.classList.remove('active');
}
}

function updateMoonPhase(now){
if(!svg.moon)return;
var dayKey=Math.floor(now.getTime()/DAY_MS);
if(state._lastMoonDay===dayKey)return;
state._lastMoonDay=dayKey;
svg.moon.textContent=MOON_PHASES[moonPhase(now)];
}
...
if(!state._cachedNearestCalc||Date.now()-state._cachedNearestCalc>30000){refreshAccentCache()}
var accent=state._cachedAccent;
var prevAccent=state._lastAccent;

var svgRoot=$('clockSvg');
if(svgRoot&&prevAccent!==accent){
svgRoot.style.setProperty('--accent',accent);
state._lastAccent=accent;
}

if(svg.ring)svg.ring.style.strokeDashoffset=SEC_RING_CIRC*(1-(s+ms/1000)/60);
if(svg.ringStop1)svg.ringStop1.setAttribute('stop-color',accent);
if(svg.ringStop2)svg.ringStop2.setAttribute('stop-color',lightenHex(accent,0.3));

var currentSeg=Math.floor(h/2);
if(svg.hourSegments&&(state._lastHourSeg!==currentSeg||prevAccent!==accent)){
state._lastHourSeg=currentSeg;
var paths=svg.hourSegments.querySelectorAll('path');
for(var k=0;k<paths.length;k++){
paths[k].classList.remove('completed','current','future');
if(k<currentSeg)paths[k].classList.add('completed');
else if(k===currentSeg)paths[k].classList.add('current');
else paths[k].classList.add('future');
}
}

if(svg.ringBgs){
for(var r=0;r<svg.ringBgs.length;r++){
...
updateEventArrow();
updateActiveEventDot();
updateMoonPhase(now);
}

function flipClock(){
var glass=$('clockGlass');
if(!glass||glass.classList.contains('day-flip'))return;
glass.classList.add('day-flip');
setTimeout(function(){glass.classList.remove('day-flip')},1000);
}
...
if(lEl&&info.items){lEl.innerHTML=info.items.map(function(i){return '<li>'+esc(i)+'</li>'}).join('')}
})
.catch(function(){
var l=$('changelogList');
if(l)l.innerHTML='<li>Версия '+(typeof APP_VERSION!=='undefined'?APP_VERSION:'2026.09.14.1')+'</li>';
});
}
...
if(btn.tagName==='BUTTON'&&!btn.innerHTML.trim())btn.innerHTML=ICONS.close;
});
...
svg.dayDots=$('dayDots');
['timerList','emptyState','emptyBtn','timerModal','timerForm','timerTitle','timerDate',
'timerType','mTitle','titleCnt','fModal','sModal','notif','importFile',
'clockSection','saveMoreBtn','saveMoreRow','resetTimeBtn',
...
'praiseInterval','heroAddBtn','heroCta'
].forEach(function(k){el[k]=$(k)});
el.empty=el.emptyState;
}

function initCardSwipe(){
if(!isTouch||!el.timerList)return;
...
if(!isTouch||!el.scrollPage)return;
...
if(dy>120){haptic('medium');closeTopModal()}
...
el.timerList.addEventListener('pointerdown',function(e){
var handle=e.target.closest('.tc-handle');
if(!handle)return;
if(e.pointerType==='mouse')handle.setAttribute('draggable','true');
else handle.removeAttribute('draggable');
});
...
if(!related||!el.timerList.contains(related))clearDragIndicators();
});
...
if(!target){
var srcIdx=state.timers.findIndex(function(x){return x.id===state._dragSrcId});
if(srcIdx!==-1&&srcIdx!==state.timers.length-1){
var moved=state.timers.splice(srcIdx,1)[0];
state.timers.push(moved);
saveTimers();render();
}
clearDragIndicators();
return;
}
if(target.dataset.id===state._dragSrcId){clearDragIndicators();return}
var r=target.getBoundingClientRect();
var insertBefore=e.clientY<r.top+r.height/2;
reorderTimers(state._dragSrcId,target.dataset.id,insertBefore);
clearDragIndicators();
});

el.timerList.addEventListener('dragend',function(){
document.body.classList.remove('dragging-active');
var dragged=el.timerList.querySelector('.tc-card.dragging');
if(dragged)dragged.classList.remove('dragging');
var handles=el.timerList.querySelectorAll('.tc-handle[draggable]');
for(var i=0;i<handles.length;i++)handles[i].removeAttribute('draggable');
...
if(el.datePickerBtn)el.datePickerBtn.addEventListener('click',function(){showPickerFallback(el.timerDate)});
...
if(e.key==='Escape'&&state._modalStack.length){closeTopModal()}
...
if(state._modalStack.length)closeTopModal(true);
}
});
...
if(PLATFORM.isIOS&&!PLATFORM.isStandalone){toast.info(TEXTS.iosInstall);return}
if(!state._installPrompt){toast.info('Приложение уже установлено или установка недоступна');return}
...
if(el.titleCnt)el.titleCnt.textContent='0/'+MAX_TITLE;
setEmptyState('empty');

setTimeout(function(){if(el.splash)el.splash.classList.add('hide')},1700);
setTimeout(function(){if(el.splash)el.splash.style.display='none'},2200);
